...
// The ctlptl tool transitively pulls in github.com/spf13/viper v1.4.0 (via
// tilt-dev/localregistry-go -> cobra@v1.0.0), which requires
// github.com/ugorji/go v1.1.4, whose bundled `codec` package collides with the
// separate github.com/ugorji/go/codec module used by gin, breaking
// `go work vendor` during codegen with an "ambiguous import". Excluding that
// version drops the (otherwise-unimported) parent module from the graph,
// eliminating the duplicate codec package.
//
// Once ctlptl is updated to a release that no longer drags in this ancient
// viper/ugorji chain, this exclude will no longer be required and can be
// removed.
...
require (
	...
	github.com/tilt-dev/ctlptl v0.9.0 // indirect
